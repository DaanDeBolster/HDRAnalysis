# Python program to analyse test sequences given its subjective test results.

# Make sure the following modules are installed before running the code:
#     pip3 install pandas
#     pip3 install numpy
...
numpart = 22
numseq = 40
scores_vid = np.array(read_csv('results/opinion_scores_vid.csv'), dtype=float)[:,1:]
scores_im = np.array(read_csv('results/opinion_scores_im.csv'), dtype=float)[:,1:]
...
for i in range(8):
    ...
    for crf in [15,20,25,30,35]:
        with open('out/seq'+str(i+1)+'/CRF_'+str(crf)+'/_PSNR_SSIM.txt') as f:
            file = f.readlines()
            line = file[36+2+240]
    ...
    fig.tight_layout()
    fig.savefig('plots/results/MOS_seq'+str(i+1)+'.png', bbox_inches='tight')
    ...
    # Generate table for thesis paper
    for crf in [15,20,25,30,35]:
        MOS_table_vid.append([
            ...
            np.round(np.nansum(scores_vid[:,int(5*i+crf/5-3)]<3)/amount_values_vid[int(5*i+crf/5-3)]*100,2),    # Percent <3
            np.round(psnr[int(crf/5-3)],2),                                                                     # PSNR
            np.round(ssim[int(crf/5-3)],2),                                                                     # SSIM
            np.round(vif[int(crf/5-3)],2),                                                                      # VIF
            np.round(vmaf[int(crf/5-3)],2),                                                                     # VMAF
            np.round(hdrvqm[int(crf/5-3)],2)                                                                    # HDRVQM
        ])
        MOS_table_im.append([
            i+1,                                                                                                # Condition (seq)
            crf,                                                                                                # Condition (crf)
            ...
            np.round(np.nansum(scores_im[:,int(5*i+crf/5-3)]<3)/amount_values_im[int(5*i+crf/5-3)]*100,2),      # Percent <3
            np.round(psnr_im[int(crf/5-3)],2),                                                                  # PSNR
            np.round(ssim_im[int(crf/5-3)],2),                                                                  # SSIM
            np.round(vif_im[int(crf/5-3)],2),                                                                   # VIF
            np.round(vmaf_im[int(crf/5-3)],2)                                                                   # VMAF
        ])
print('Results table videos:')
print()
...
print("------------------------------------------------------------------------------------------")
print()
print('Results table still images:')
print()
for i in range(40):
    if i%5==0:
        print('\t\hline')
...
fig.savefig('plots/results/Correlation_video_1.png', bbox_inches='tight')
...
print('Video-image correlation coefficient:')
print()
...
fig.savefig('plots/results/Correlation_video_2.png', bbox_inches='tight')
...
fig.savefig('plots/results/Correlation_images_1.png', bbox_inches='tight')
fig, axs = plt.subplots(1, 2, figsize=(6, 3))
...
fig.savefig('plots/results/Correlation_images_2.png', bbox_inches='tight')